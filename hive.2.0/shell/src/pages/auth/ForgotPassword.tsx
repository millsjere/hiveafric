...
import BgImage from '../../assets/images/colors.jpg'
...
const PickItem = ({ icon, title, copy, onClick }: PickProps) => {
    return (
        <Box onClick={onClick} mb={2} sx={{ cursor: 'pointer', ':hover': { '#icon': { color: '#ED8A2F' }, border: theme => `1px solid ${theme.palette.primary.main}` } }} p={3} borderRadius={'10px'} border={'1px solid lightgrey'}>
            {icon}
...
const ForgotPassword = () => {
    ...
    const onFormSubmit = async () => {
        ...
        try {
            setLoad(true)
            const { data: res } = await base.post('/hive/user/forgot-password', { type: pick, email, sms: phone })
